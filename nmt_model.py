...
class NMT(nn.Module):
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        encoder_hidden_proj = self.att_projection(enc_hiddens)
        Y = self.model_embeddings.target(target_padded)
        time_steps = torch.split(Y, 1)
        for Y_t in time_steps:
            Y_t = torch.squeeze(Y_t)
            Ybar_t = torch.cat((Y_t, o_prev), 1)
            (next_state, next_cell), o_t, _ = self.step(Ybar_t, dec_state, enc_hiddens, encoder_hidden_proj, enc_masks)
